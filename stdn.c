...
/**
 * err - it prints appropiate error messages based on error code
 * @error_code: The error codes are the listed:
 * (1) => user does not give any file or more than one file to the program
 * (2) => file provided can't be opened or read
 * (3) => file contains an invalid instruction
 * (4) => the program is not to malloc more memory
...
/**
 * more_err - aid in handling errors
 * @error_code: The error codes are the listed:
...
/**
 * string_err - aids in handlind errors
 * @error_code: error codes are the listed:
